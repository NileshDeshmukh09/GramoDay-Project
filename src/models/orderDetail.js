...
function findOrderByUser(data, cb){
    var sql =`SELECT ID, TotalPrice AS Total
              FROM orderdetail
              WHERE UserId =? 
              LIMIT 1 `;
    var values=[];
...
function addOrder(data,cb){
    var sql =`INSERT INTO orderdetail
...
function getOrderDetailsByUserID(data, cb){
    var sql =`
    SELECT OD.ID as OrderID , C.CmdtyName as ComdtyName,  OI.PriceUnit as Priceunit,
    M.MarketName as marketName , M.MarketType as MarketType , OI.ConvFactor as convFactor,
    OD.TotalPrice AS price
    FROM orderdetail OD LEFT JOIN orderItem OI
    ON  OD.ID = OI.OrderID LEFT JOIN markets M
    ON OI.MarketID = M.ID LEFT JOIN commodity C
    ON OI.CmdtyID = C.ID
    WHERE  OD.UserID = ?; `;
...
function getOrderDetails( cb){
    var sql =`
    SELECT OD.ID as OrderID , OD.UserID, U.UserName, C.CmdtyName as ComdtyName,  OI.PriceUnit as Priceunit,
    M.MarketName as marketName , M.MarketType as MarketType , OI.ConvFactor as convFactor,
    OD.TotalPrice AS price
    FROM orderdetail OD LEFT JOIN orderItem OI
    ON  OD.ID = OI.OrderID LEFT JOIN markets M
    ON OI.MarketID = M.ID LEFT JOIN commodity C
    ON OI.CmdtyID = C.ID LEFT JOIN  users U
    ON U.ID = OD.UserID; 
    `;
